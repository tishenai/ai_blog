...
> **当前版本**：v1.7.3（2026-06-30 新增 rewrite 前的格式理解规则：先读参考样本，禁止凭印象重写）
> **上一版**：v1.7.2（2026-06-30 新增 rewrite 安全规则）
...
│  每天 17:00, agentTurn + 绑定飞书 DM │
│  thinking: xhigh, model: doubao-seed-2.0-pro │
└──────────────┬──────────────────────┘
               │
               ▼
┌─────────────────────────────────────┐
│  AI agent (替身，绑定到飞书 DM 会话)  │
│  ① 读 topic_pool.md 选 pending 话题  │
│  ② 用最高思考模式写 ~4000-6000 字正文 │
│  ③ 写到 pending/<slug>.md           │
...
│  每天 17:30, agentTurn + 绑定飞书 DM │
│  轻量兜底，不写文章                  │
│  ① 扫描 pending 草稿 + wiki Draft   │
│  ② pending 无 Draft 时创建审稿文档   │
│  ③ 重建知识库首页                    │
│  ④ 对未通知 Draft 补发审稿提醒       │
│  ⑤ audit 通过才算完成               │
└─────────────────────────────────────┘
                                          ║
                              owner 在飞书 IM 私聊回复"过了"或 /publish
                                          ║
                                          ▼
┌──────────────────────────────────────────────────────────┐
│  main session → 直接执行完整发布流程                      │
│  1. 写入 .publish_params.json                           │
│  2. run_publish.py（git mv + build + commit + push）     │
│  3. 更新飞书文档为 Published 全文                        │
│  4. 刷新 Wiki 首页                                       │
│  5. 发发布通知                                           │
│  6. 归档状态文件                                         │
│  ⚠️ publish-blog-post cron 已禁用（enabled=false）        │
└──────────────────────────────────────────────────────────┘
```
...
schedule:
  ...
  expr: '0 17 * * *' # 每天 17:00
  tz: Asia/Shanghai
sessionTarget: main # 改为 main session systemEvent，解决 isolated git push 静默失败问题
wakeMode: now
payload:
  kind: systemEvent
  message: |
    ...
    3. 用最高思考模式写 3500-4500 字到 pending/<slug>.md
       ...
       - **【rewrite 格式理解规则】对陌生格式动手之前，必须先读 2-3 篇正确参考文章（posts/ 目录下已确认格式正确的），搞清楚结构：声明段落在哪个位置、"读不到的东西"是节标题还是自然段落、frontmatter 每一篇怎么写。禁止凭表面印象乱猜后直接覆写。**
       ...
       - 必须有"读不到的东西"段落
       - 文末必须加 AI 声明（写在正文最后一个段落之后、分隔线之前），措辞视选题来源而定：
         - pick_topic.py 自动选的题（自主选题）：加「这篇文章由本博客的 AI 作者（替身）生成，选题来自每日 AI 自动选题流程，未经人类作者改写主体内容。」
         - 用户通过 .daily_ai_blog_manual_topic.md 指定的话题（用户给定）：加「这篇文章由本博客的 AI 作者（替身）生成，选题来自用户指定，未经人类作者改写主体内容。」
         - 注意：声明段落本身不需要加*斜体*，正文自然段落即可；分隔线---放在声明之后。格式：正文最后一段 → [声明段落] → ---（分隔线，不标数字编号）
...
payload:
  ...
  timeoutSeconds: 900
  message: |
    【daily-ai-blog-postflight 触发】这是 daily-ai-blog-post 的结果验收与兜底任务。不要写文章，不要改文章正文。

    只执行以下轻量步骤：
    1. feishu_wiki_space_node list(space_id='7650738808775330774', page_size=50) 取所有节点。
    2. 精简为 [{node_token,title,url}] 写入 /tmp/wiki_nodes.json。
    3. cd /root/.openclaw/workspace/ai_blog && python3 tools/daily_post/draft_postflight.py plan --nodes-json /tmp/wiki_nodes.json --render-missing-review-dir /tmp/daily-blog-review-docs > /tmp/draft_postflight_plan.json。
...
### 任务 3：publish-blog-post（已禁用）

> ⚠️ **此 cron 已禁用（enabled=false），不会再触发。**
> 发布路径：owner 在飞书 DM 说 `/publish <slug>` → main session 直接执行完整发布流程（见下方「/publish 主路径」）

```yaml
name: publish-blog-post
enabled: false # 已禁用，发布走 main session 直接执行
schedule:
  kind: cron
  expr: '0 0 31 12 *' # 不可能触发的时间
```
...
### v1.5.4 daily draft postflight 结果驱动验收（2026-06-14）

事故复盘：`daily-ai-blog-post` 写出 `pending/*.md` 后没有主动给 owner 发送飞书审稿通知，owner 需要询问“写成啥样了”才发现草稿已存在。进一步排查发现 `daily-ai-blog-postflight` 使用 `main + systemEvent` 后，cron 运行记录显示 `ok` 但 `durationMs=5ms`，只代表 systemEvent 被记录/投递，不代表主会话真的执行了列 wiki、更新首页、补发通知等步骤。这与已废弃的 `publish-blog-post` 假成功属于同类问题。

修复原则：daily draft postflight 不能再信任 cron `ok`，必须以结果验收为准。

已调整：

- `daily-ai-blog-postflight` 改为 `isolated + agentTurn` 的轻量执行任务，不再使用 `main + systemEvent`。
- `tools/daily_post/draft_postflight.py plan` 不只扫描 wiki Draft，也扫描本地 `pending/*.md`：
  - `pending_without_wiki_draft_count > 0` 表示草稿已生成但飞书 Draft 文档缺失，postflight 必须创建 Draft 文档。
  - `needs_notification_count > 0` 表示 Draft 文档存在但审稿通知未记录，postflight 必须补发标准通知并 `mark-notified`。
- 新增 `audit` 子命令：只要存在“pending 草稿无 Draft 文档”或“Draft 未通知”，就返回非 0，禁止把 postflight 判为成功。
- 新增测试 `tests/test_draft_postflight.py`，覆盖“pending 有草稿但无 Draft 文档”和“Draft 存在但未通知”两类漏闭环场景。

验收口径：只有 owner 收到可点击飞书审稿文档 + 可复制 `/publish <slug>` 命令，并且 audit 通过，daily draft 才算真正完成。

### v1.5.5 postflight E2E 验证记录（2026-06-14）

按真实端到端路径验证 `daily-ai-blog-postflight`：

- 测试草稿：`pending/postflight-e2e-test-20260614215111.md`
- 测试 Draft 文档：`[Draft 2026-06-14] Postflight E2E 测试草稿 215111`
- 飞书 wiki node：`N5RjwEJnWihBW8kNacaczxekn4f`
- 手动触发 cron run：`manual:f408e930-3495-4d06-8110-4171d0c3b32f:1781445077123:6`
- 运行耗时：`100876ms`，不是 `5ms` systemEvent 假成功。
- cron summary：`pending_count=1`、`created_doc_count=1`、`draft_count=1`、`补发通知数量=1`、`audit=通过`。
- delivery：`delivered=true`，并确认 `messageToolSentTo` 指向 owner 飞书私聊。

验证结论：`isolated + agentTurn` postflight 能在“本地已有 pending 草稿但没有飞书 Draft/通知”的情况下，创建 Draft 审稿文档、补发审稿通知、写入 `mark-notified` 状态并通过 `audit`。这覆盖了 2026-06-14 暴露的“写完不主动通知，需要 owner 主动问”的核心失败模式。

后续注意：E2E 测试会留下测试 Draft 文档和测试 pending 草稿，清理时必须明确按测试 slug 删除/移动，避免影响真实文章。

### v1.6.0 publish-blog-post cron 正式禁用 + 文档与实际状态对齐（2026-06-22）

**本次变更**：

- `publish-blog-post` cron 正式禁用（`enabled=false`），schedule 改为不可能触发的 `0 0 31 12 *`。
- 发布主路径确认：owner 在飞书 DM 说 `/publish <slug>` 后，main session **直接执行**完整发布流程，不再依赖任何 cron。
- `daily-ai-blog-post` 实际使用 `agentTurn + 绑定飞书 DM 会话`（`sessionTarget=session:agent:...:direct:...`），WORKFLOW 架构图和 cron 定义同步更新。
- `daily-ai-blog-postflight` 同样使用 `agentTurn + 绑定飞书 DM 会话`，不另开 isolated session（避免通知发到错误目标）。
- WORKFLOW.md 变更历史新增 v1.6.0 条目。

**当前三个 cron 运行状态**：

| 任务                       | enabled  | 触发时间   | 实际行为                             |
| -------------------------- | -------- | ---------- | ------------------------------------ |
| `daily-ai-blog-post`       | ✅ true  | 每天 17:00 | 写稿 + 全链路收尾                    |
| `daily-ai-blog-postflight` | ✅ true  | 每天 17:30 | 兜底校验 + 补发通知                  |
| `publish-blog-post`        | ❌ false | 不触发     | 已禁用，发布走 main session 直接执行 |

---

### v1.5.7 daily-ai-blog-post 从 isolated 改为 main systemEvent（2026-06-15）

**根因**：isolated session 执行 git push 时，如果 SSH 推送静默失败（如工作目录状态问题），cron 仍报 `ok` 和 `delivered=true`。postflight 看到 pending 目录为空（因为根本没 push 成功），也报 `ok` 并结束。两层都"成功"了，但文章根本没写进仓库。

**修复**：

- `daily-ai-blog-post` 从 `sessionTarget: isolated + payload.kind: agentTurn` 改为 `sessionTarget: main + payload.kind: systemEvent`
- 写稿任务现在在 main session 直接执行，git push 结果直接可见，不再有"静默失败"的盲区
- 原理同 v1.5.1 的 publish-blog-post 修复：多系统流程必须以 main session 执行，避免 isolated agent 的不可观测性问题

**验证**：

- `lastRunStatus=ok` + `lastDurationMs=104584ms`（约104秒），说明 main session 真正执行了写稿流程，不是假成功
- 本次手动重跑成功写出了 `pending/conversations-i-was-not-invited-to.md`（2243字）并完成全流程

**后续注意**：如果 main session 在 17:00 被用户对话占用，systemEvent 会在用户对话结束后立即执行（不排队），仍会触发写稿。

将 postflight 从 agent 自由发挥 prompt 改为确定性脚本调用。

新增文件：`tools/daily_post/postflight_runner.py`

暴露 5 个子命令供 agent 调用：

- `list-nodes`：验证 `/tmp/wiki_nodes.json` 存在，输出节点数量
- `plan`：调用 `draft_postflight.py plan`，生成 postflight plan JSON
- `send-notifications`：读取 plan，打印需要通知的 draft JSONL，agent 负责调用飞书工具
- `mark-notified`：对单个 slug 调用 `draft_postflight.py mark-notified`
- `audit`：调用 `draft_postflight.py audit`，exit 0 = 通过

cron prompt 也改为严格步骤式。agent 只负责：

1. 调用 `feishu_wiki_space_node list` 获取节点
2. 将节点写入 `/tmp/wiki_nodes.json`
3. 调用 `postflight_runner.py list-nodes` 验证
4. 调用 `postflight_runner.py plan` 生成 plan
5. 根据 plan 结果，决定是否需要创建 Draft（调用 `feishu_create_doc`）
6. 调用 `build_wiki_index.py` + `feishu_update_doc` 刷新首页
7. 读取 plan，调用 `feishu message` 发送通知
8. 对每个发送成功的调用 `mark-notified`
9. 调用 `audit` 验证通过

优点：

- 步骤可预期，不依赖 agent 理解自然语言指令
- 出错后重试只需要重跑特定步骤，不需要从头开始
